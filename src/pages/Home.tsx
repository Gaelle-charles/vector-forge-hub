...
            ref.current?.pause();
            ref.current.muted = true; // Mute all videos
          });
          videoRefs.current[nextVideo].current?.play().catch(e => console.log("Autoplay prevented:", e));
          videoRefs.current[nextVideo].current.muted = false; // Unmute the active video
          return nextVideo;
        });
      }, 4000);
      return interval;
...
    videoRefs.current[0].current.muted = false; // Unmute the first video on load
...
    videoRefs.current[index].current.muted = false; // Unmute the active video
  };
